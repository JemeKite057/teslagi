...
list_doctor = ['', 'dr. Nurita', 'dr. Yogi', 'dr. Wibowo', 'dr. Ulama', 'dr. Ping', 'akmal']
...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS datar (id serial, doctor_name varchar, patient_name varchar, gender char(25), \
...
if page == "View Data":
    data = conn.query('SELECT * FROM datar ORDER By id;', ttl="0").set_index('id')
    st.dataframe(data)

if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO datar (doctor_name, patient_name, gender, symptom, handphone, address, waktu, tanggal) \
                          VALUES (:1, :2, :3, :4, :5, :6, :7, :8);')
            session.execute(query, {'1':'', '2':'', '3':'', '4':'[]', '5':'', '6':'', '7':None, '8':None})
            session.commit()

    data = conn.query('SELECT * FROM datar ORDER By id;', ttl="0")
            ...
            with st.form(f'data-{id}'):
                ...
                with col2:
                    if st.form_submit_button('DELETE'):
                        query = text(f'DELETE FROM datar WHERE id=:1;')
